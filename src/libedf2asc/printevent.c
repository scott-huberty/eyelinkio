...
int print_event(FEVENT *fevent, int sample_int)
{
  ...
  switch(etype)
  {
	  ...
	  case ENDSACC:
		...
		{
			case OUTPUT_PUPIL:
			print_value(fevent->gstx, 1);
			print_value(fevent->gsty, 1);
			print_value(fevent->genx, 1);
			print_value(fevent->geny, 1);
			if(fevent->read & READ_GRES)   /* CHANGED FOR REAL SACCADE AMPLITUDE */
			{
				print_value(hypot(
					(fevent->gstx-fevent->genx)/(((fevent->supd_x+fevent->eupd_x)/2.0)),
					(fevent->gsty-fevent->geny)/(((fevent->supd_y+fevent->eupd_y)/2.0))), 2);
	...
	break;
	  ...
	  case ENDFIX:
		...
		switch(options.output_event_type)
		{
			...
			case OUTPUT_GAZE:
			default:
			  print_value((fevent->gavx), 1);
			  print_value((fevent->gavy), 1);
			  print_value(fevent->ava,  0);
		}
		if(options.output_resolution)
			switch(options.output_event_type)
			{
				case OUTPUT_HREF:
					...
					{
						float x1 = (fevent->gavx);
						float y1 = (fevent->gavy);
						float dx = (float)((15000.0*15000.0+x1*x1+y1*y1)/
							sqrt(15000.0*15000.0+y1*y1));
						float dy = (float)((15000.0*15000.0+x1*x1+y1*y1)/
							sqrt(15000.0*15000.0+x1*x1));

 ...
 * Copyright (c) 1996-2024 SR Research Ltd.
 *
...
#define EDF_ERROR  -1
#define EDF_OK     0
#define MAX_FILE_NAME_SIZE 512 // maximum characters for file name
...
int rename_extension(char *dest, char *src);
...
void init_options()
{
	...
	options.disable_pa_check = 0;


	options. simulation_screen_distance= 700;
	options. simulation_screen_distance_bot = 760;
	// screen_phys_coords : L, T, R, B
	options. screen_phys_l = -200.0;
	options. screen_phys_t =  150.0;
	options. screen_phys_r =  200.0;
	options. screen_phys_b = -150.0;
	// screen_pixel_coords : L, T, R, B
	options. screen_pixel_l = 0.0;
	options. screen_pixel_t = 0.0;
	options. screen_pixel_r = 1023.0;
	options. screen_pixel_b = 767.0;
	options. sepres;
...
int main (int argc, char ** argv)
{
	...
	printf("Copyright (c) 1996-2024 SR Research Ltd., last modified %s\n\n", __DATE__);
...
int process_edf_file(char * file, char *outfile)
{
	...
	ed = edf_open_file(edffile, options.enable_consistency_check|(options.enable_failsafe?1<<2:0)|(options.disable_large_time_stamp_check?1<<3:0),
		(options.logmsg?EDF_LOAD_EXTRA_MSG:EDF_LOAD_EVENTS               //api-239: replace options.events_enabled with 1
		),(options.samples_enabled),&rv); /* open the edf file */
	...
	*/
	...
	if(ed != NULL)
	{
		...
		*/
		...
		edf_get_trial_count(ed); // force edfapi to parse trials.
...
void print_usage(char * program)
{
	  ...
	  printf("-npa_check disable pupil area check to get gaze, href and raw data. \n");
	  printf("-logmsg load eb_message.log to integrate with edf file. \n");

#ifdef DISABLE_FOR_RELEASE
	  printf("-sepres distance,distance_bot,phys L,phys T,phys R,phys B,pixel L,pixel T,pixel R,pixel B\n");
	  printf("        The above values are from the following eyelink commands:\n");
	  printf("        1. screen_distance (2 values)\n");
	  printf("        2. screen_phys_coords (4 values)\n");
	  printf("        3. screen_pixel_coords (4 values)\n");
	  printf("        4. requires both gaze and href sample data.\n");
	  printf("        Remote mode and head mounted systems are not supported.\n");
	  printf("        eg. edf2asc -sepres 700,760,-200,150,200,-150,0,0,1023,767\n");

#ifdef EDF_REPARSER
	  printf("-reparse <config> reparse edf file with new configuration\n");
#endif
	...
	for(i=1;i<argc;i++)
	{
		if( argv[i][0]=='-')
		{
			switch(toupper(argv[i][1]))
			{
				...
				case 'R':		/* set right-only or other*/
				...
				case 'S':
					if(!stricmp(argv[i],"-SEPRES"))/* separate resolutions  */
						{
							int rv =0;
							i++;
							if(i>argc) break;
							rv = sscanf(argv[i],"%f,%f,%f,%f,%f,%f,%f,%f,%f,%f",
								&(options.simulation_screen_distance),
								&(options.simulation_screen_distance_bot),
								// screen_phys_coords : L, T, R, B
								&(options.screen_phys_l),
								&(options.screen_phys_t),
								&(options.screen_phys_r),
								&(options.screen_phys_b),

								// screen_pixel_coords : L, T, R, B
								&(options.screen_pixel_l),
								&(options.screen_pixel_t),
								&(options.screen_pixel_r),
								&(options.screen_pixel_b));
							if(rv != 10)
								break;
							else
								options.sepres = 1;
				...
				case 'N':
					switch(toupper(argv[i][2]))
					{
						...
						case 'P':
							{
								if(toupper(argv[i][3])=='A')
									options.disable_pa_check = 1;
							}
...
/***********************************************************************
 *Function Name: set_file_name
 *Input: filename, provided_filename, is_edf
 *Output: failure/success : -1/0
 *Purpose: Allocate memory and copy the provided filename to file_name with forward slashes.
 *       If is_edf is set, also check file name ends with .edf otherwise return -1
 ***********************************************************************/
int set_file_name(char ** file_name, char * provided_filename, int is_edf)
{
	*file_name = (char *)malloc(sizeof(char ) * (strlen(provided_filename)+1));
	strcpy(*file_name,provided_filename);
	replace(*file_name,'\\','/');
	if (is_edf != 0)
	{
		//Make sure extention is .edf
		if(is_edf_file(*file_name) != 1)
		{
			printf("\nERROR: file is not .EDF file\n\n");
			return -1;
		}
	}
	return 0;
